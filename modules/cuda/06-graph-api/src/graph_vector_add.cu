...
int main(int argc, char** argv) {
    int n = 1 << 20;                       // количество элементов (по умолчанию 1M)
    if (argc == 2) n = std::atoi(argv[1]);
    size_t bytes = n * sizeof(float);      // объём данных в байтах

    /***** 1. Host память *****/
    std::vector<float> h_a(n, 1.0f), h_b(n, 2.0f), h_c(n); // host-векторы

    /***** 2. Device память *****/
    float *d_a, *d_b, *d_c;                // device-указатели
    check(cudaMalloc(&d_a, bytes), "malloc a"); // выделяем память на GPU
    check(cudaMalloc(&d_b, bytes), "malloc b");
    check(cudaMalloc(&d_c, bytes), "malloc c");

    /***** 3. Создаём пустой граф *****/
    cudaStream_t captureStream;            // стрим, в котором будем захватывать граф
    cudaStreamCreate(&captureStream);
    cudaGraph_t graph;
    cudaGraphExec_t graphExec;

    // --- Запись операций ---
    check(cudaStreamBeginCapture(captureStream, cudaStreamCaptureModeGlobal), "begin capture"); // старт захвата
    ...
    // Запуск ядра
    int blocks = (n + THREADS - 1) / THREADS; // количество блоков для ядра
    ...
    check(cudaStreamEndCapture(captureStream, &graph), "end capture"); // конец захвата, получили граф

    // --- Инстанцируем граф ---
    check(cudaGraphInstantiate(&graphExec, graph, nullptr, nullptr, 0), "instantiate");

    /***** 4. Многократный запуск графа *****/
    const int iters = 100; // число повторов запуска графа
    for (int i = 0; i < iters; ++i) {
        check(cudaGraphLaunch(graphExec, captureStream), "launch graph");
        check(cudaStreamSynchronize(captureStream), "sync"); // ждём выполнения графа
    }

    /***** 5. Проверка корректности *****/
    bool ok = true;
    for (int i = 0; i < n; ++i) {
        if (fabs(h_c[i] - 3.0f) > 1e-5f) { ok = false; break; } // проверяем результат (1+2)
    }
    printf("Graph VectorAdd %s | iters=%d, elements=%d\n", ok ? "PASSED" : "FAILED", iters, n);

    /***** 6. Очистка *****/
    cudaGraphExecDestroy(graphExec);       // освобождаем ресурсы
